import os
from src.tflite_bit import tf_float32_pack, tf_int8_pack, judge_tflite_int8
from src.onnx_bit import onnx_float32_pack, onnx_int8_pack, judge_onnx_int8
...
def tflite_deploy(cover_model, data_format):
    if judge_tflite_int8(cover_model):
        cover_model = tf_int8_pack(cover_model)
    else:
        cover_model = tf_float32_pack(cover_model)
    return cover_model



def onnx_deploy(cover_model, data_format):
    if judge_onnx_int8(cover_model):
        cover_model = onnx_int8_pack(cover_model)
    else:
        cover_model = onnx_float32_pack(cover_model)
    return cover_model
